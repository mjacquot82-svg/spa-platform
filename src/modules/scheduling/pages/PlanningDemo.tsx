...
import { CatalogItemService, InMemoryCatalogItemRepository } from '../../catalog';
import {
...
const businessId = 'planning-business';
const providerId = 'provider-michelle';
const periodsSeed: PlanningPeriod[] = [
  { id: 'period-july', businessId, year: 2026, month: 7, status: 'published' },
  { id: 'period-august', businessId, year: 2026, month: 8, status: 'published' },
  { id: 'period-september', businessId, year: 2026, month: 9, status: 'draft' },
  { id: 'period-october', businessId, year: 2026, month: 10, status: 'archived' },
];
const planningPeriods = new PlanningPeriodService(new InMemoryPlanningPeriodRepository(periodsSeed));
const workingHours = new WorkingHoursService(new InMemoryWorkingHoursRepository(Array.from({ length: 7 }, (_, dayOfWeek) => ({ id: `hours-${dayOfWeek}`, businessId, resourceId: providerId, dayOfWeek, enabled: dayOfWeek > 0 && dayOfWeek < 6, timeRanges: [{ startTime: '09:00', endTime: '17:00' }] }))));
const exceptions = new AvailabilityExceptionService(new InMemoryAvailabilityExceptionRepository());
const catalog = new CatalogItemService(new InMemoryCatalogItemRepository([{ id: 'treatment-planning', businessId, type: 'Service', name: 'Signature Massage', description: '', category: 'Massage', image: null, active: true, durationMinutes: 60, bufferBeforeMinutes: 0, bufferAfterMinutes: 0, resourceTypesRequired: ['staff'], createdAt: '2026-07-01', updatedAt: '2026-07-01', deletedAt: null }]));
const resources = new SchedulingResourceService(new InMemorySchedulingResourceRepository([{ id: providerId, businessId, name: 'Michelle', type: 'staff', active: true, metadata: {} }]));
const appointments = new AppointmentService(new InMemoryAppointmentRepository());
const scheduling = new SchedulingService(catalog, resources, workingHours, exceptions, appointments, planningPeriods);
...
export default function PlanningDemo() {
  ...
  const refresh = useCallback(async () => {
    const [nextPeriods, nextHours] = await Promise.all([planningPeriods.listPeriods(businessId), workingHours.listWorkingHoursForResource(businessId, providerId)]);
  ...
  useEffect(() => {
    if (!selected) { setPeriodExceptions([]); setPreview([]); return; }
    ...
    void Promise.all([
      ...
      scheduling.findNextAvailableAppointments({ businessId, catalogItemId: 'treatment-planning', preferredResourceId: providerId, preferredDate: start.slice(0, 10), numberOfSuggestions: 3 }),
